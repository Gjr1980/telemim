...
export default function App(){
  ...
  const [rastreioList,setRastreioList]=useState([]);
  const [rastreioLoading,setRastreioLoading]=useState(false);
  const [rastreioCobrando,setRastreioCobrando]=useState(false);
  async function loadRastreio(){setRastreioLoading(true);try{var r=await fetch(SUPA_URL+"/rest/v1/v_status_rastreamento?select=*",{headers:getH()});if(r.ok){var d=await r.json();setRastreioList(Array.isArray(d)?d:[]);}}catch(e){}setRastreioLoading(false);}
  ...
  useEffect(()=>{
    ...
    getSupaClient().then(function(sb){
      ...
      canal=sb.channel('telemim-live')
        ...
        .on('postgres_changes',{event:'INSERT',schema:'public',table:'solicitacoes_financeiras'},function(p){
          if(p.new){
            setSolicitacoesFin(function(prev){return [p.new].concat(prev.filter(function(s){return s.id!==p.new.id;}));});
            try{
              var _u=JSON.parse(localStorage.getItem('tmim_u')||'null');
              if(_u&&_u.perfil==='admin'&&p.new.status==='pendente'){
                var _tipo=p.new.tipo==='editar_valor'?'✏️ Edição':p.new.tipo==='remover_dia'?'🗑️ Remoção de dia':'🗑️ Remoção';
                setToastMsg(_tipo+' solicitada por '+(p.new.supervisor_nome||'?')+' — '+(p.new.prestador_nome||p.new.ajudante_nome||''));
                setTimeout(function(){setToastMsg('');},6000);
                if('Notification' in window&&Notification.permission==='granted'){
                  try{new Notification('📩 Nova solicitação',{body:_tipo+' — '+(p.new.supervisor_nome||'?'),icon:'/icons/icon-192.png'});}catch(_){}
                }
              }
            }catch(_){}
          }
        })
        .on('postgres_changes',{event:'UPDATE',schema:'public',table:'solicitacoes_financeiras'},function(p){
          if(p.new){setSolicitacoesFin(function(prev){return prev.map(function(s){return s.id===p.new.id?p.new:s;});});}
        })
  ...
  function gpsStart(agendaId,veiTipo){
    ...
    // If first time — show OwnTracks install prompt
    if(!localStorage.getItem("tmim_rastreio_skip")){
  ...
  async function gpsLoadPositions(agId,motoristaId){
    // Supabase gps_tracking (posições gravadas pelo OwnTracks em 2º plano ou pelo navegador)
    try{
    ...
    // (Traccar removido — rastreamento agora vem de gps_tracking via OwnTracks/navegador)
    return null;
  }
  ...
  async function loadAuditLixeira(){
    ...
    try{
      ...
      var [_r1,_r2]=await Promise.all([fetch(SUPA_URL+"/rest/v1/mudancas?"+_q,{headers:_h}),fetch(SUPA_URL+"/rest/v1/agenda?"+_q,{headers:_h})]);
  ...
  async function loadAuditSaude(){
    ...
    try{
      ...
      var _iso7=_fmtDate(_ini7);
      var _ini24=new Date(_hj);_ini24.setHours(_ini24.getHours()-24);
      var _iso24=_ini24.toISOString();
      var [_rm,_ra,_re]=await Promise.all([fetch(SUPA_URL+"/rest/v1/mudancas?data=gte."+_iso7+"&deleted_at=is.null&select=id,data,nome,status,medicao&limit=2000",{headers:_h}),fetch(SUPA_URL+"/rest/v1/agenda?data=gte."+_iso7+"&deleted_at=is.null&select=id,data,nome,status,medicao,supervisor_id&limit=2000",{headers:_h}),fetch(SUPA_URL+"/rest/v1/auditoria?acao=in.(trigger_sync_agenda_to_mudancas_erro,sync_erro)&criado_em=gte."+_iso24+"&select=id,criado_em&limit=500",{headers:_h})]);
      ...
      var _porDia={};
      for(var i=6;i>=0;i--){var d=new Date(_hj);d.setDate(d.getDate()-i);var k=_fmtDate(d);_porDia[k]={data:k,mud:0,m3:0};}
  ...
  async function loadAuditMorador(query){
    ...
    try{
      ...
      var [_rm,_ra]=await Promise.all([fetch(SUPA_URL+"/rest/v1/mudancas?nome=ilike."+_q+"&order=data.desc&limit=200",{headers:_h}),fetch(SUPA_URL+"/rest/v1/agenda?nome=ilike."+_q+"&order=data.desc&limit=200",{headers:_h})]);
  ...
  async function loadAuditDuplicatas(){
    setAuditLoading(true);
    try{
      var _r=await fetch(SUPA_URL+"/rest/v1/ajudantes?select=*&order=nome",{headers:getH()});
      var _ajs=_r.ok?await _r.json():[];
      var _grupos={};
      _ajs.forEach(function(aj){
        var _k=(aj.nome||"").toLowerCase().trim().replace(/\s+/g," ");
        if(!_grupos[_k])_grupos[_k]={nome:aj.nome,key:_k,membros:[]};
        _grupos[_k].membros.push(aj);
      });
      var _dups=Object.values(_grupos).filter(function(g){return g.membros.length>1;});
      var _hj=new Date();_hj.setDate(_hj.getDate()-90);
      var _isoIni=_fmtDate(_hj);
      var _re=await fetch(SUPA_URL+"/rest/v1/equipe_dia?data=gte."+_isoIni+"&select=ajudantes&limit=2000",{headers:getH()});
      var _eqs=_re.ok?await _re.json():[];
      var _diasPorId={};
      _eqs.forEach(function(e){if(Array.isArray(e.ajudantes))e.ajudantes.forEach(function(aj){if(aj&&aj.id){_diasPorId[aj.id]=(_diasPorId[aj.id]||0)+1;}});});
      _dups.forEach(function(g){g.membros.forEach(function(m){m._dias90=_diasPorId[m.id]||0;});});
  ...
  async function adminDesfazerInicio(ag){
    if(!ag)return;
    var _hj=new Date();var _hjStr=_fmtDate(_hj);
  ...
  async function executarMerge(){
    ...
    try{
      var _re=await fetch(SUPA_URL+"/rest/v1/equipe_dia?select=id,ajudantes&limit=5000",{headers:getH()});
      var _eqs=_re.ok?await _re.json():[];
      var _srcIds=m.sources.map(function(s){return s.id;});
      var _updates=[];
      _eqs.forEach(function(e){
        if(!Array.isArray(e.ajudantes))return;
        var _changed=false;
        var _new=e.ajudantes.map(function(aj){
          if(aj&&_srcIds.includes(aj.id)){_changed=true;return Object.assign({},aj,{id:m.target.id,nome:m.target.nome});}
          return aj;
        });
        if(_changed){
          var _seen={};
          _new=_new.filter(function(aj){if(!aj||!aj.id)return true;if(_seen[aj.id])return false;_seen[aj.id]=true;return true;});
          _updates.push({id:e.id,ajudantes:_new});
        }
      });
      for(var i=0;i<_updates.length;i++){
        await fetch(SUPA_URL+"/rest/v1/equipe_dia?id=eq."+_updates[i].id,{method:"PATCH",headers:Object.assign({},getH(),{"Content-Type":"application/json","Prefer":"return=minimal"}),body:JSON.stringify({ajudantes:_updates[i].ajudantes})});
      }
      for(var j=0;j<m.sources.length;j++){
        await fetch(SUPA_URL+"/rest/v1/ajudantes?id=eq."+m.sources[j].id,{method:"PATCH",headers:Object.assign({},getH(),{"Content-Type":"application/json","Prefer":"return=minimal"}),body:JSON.stringify({ativo:false})});
      }
      var _patch={};
      if(!m.target.telefone){var _withTel=m.sources.find(function(s){return s.telefone;});if(_withTel)_patch.telefone=_withTel.telefone;}
      if(!m.target.pix){var _withPix=m.sources.find(function(s){return s.pix;});if(_withPix)_patch.pix=_withPix.pix;}
      if(Object.keys(_patch).length>0){
        await fetch(SUPA_URL+"/rest/v1/ajudantes?id=eq."+m.target.id,{method:"PATCH",headers:Object.assign({},getH(),{"Content-Type":"application/json","Prefer":"return=minimal"}),body:JSON.stringify(_patch)});
      }
      setMergeAjudantes(null);
      setSyncStatus("✅ Merge concluído");
      loadAuditDuplicatas();
      loadAjudantes();
    }catch(e){alert("Erro no merge: "+e.message);try{window.Sentry&&window.Sentry.captureException(e,{tags:{op:"executarMerge"}});}catch(_){}}
  }
  function exportAuditCSV(){
    ...
    var url=URL.createObjectURL(blob);var a=document.createElement("a");a.href=url;a.download="auditoria_lixeira_"+_fmtDate(new Date())+".csv";a.click();URL.revokeObjectURL(url);
  }
    ...
    function _getActiveVehicles(){
      ...
      _all3.forEach(function(am){
        ...
        if((am.inicio_van_em||am.van_saiu_em)&&!am.chegada_van_em&&am.motorista_van_id){
          _vehs.push({agId:am.id,motId:am.motorista_van_id,veiculo:"van",destino:am.destino||""});
        }
        if((am.inicio_caminhao_em||am.caminhao_saiu_em)&&!am.chegada_caminhao_em&&am.motorista_caminhao_id){
          _vehs.push({agId:am.id,motId:am.motorista_caminhao_id,veiculo:"cam",destino:am.destino||""});
        }
      });
      return _vehs;
    }
  ...
  async function gerarMagicLink(dataAlvo){
    ...
    try{
      var dt=dataAlvo||_fmtDate(new Date());
  ...
  async function gerarPDFConsolidado(lista,dataIni,dataFim,btnRef){
    ...
    try{
      ...
      doc.save('Relatorio_Consolidado_TELEMIM_'+_fmtDate(now)+'.pdf');
    }finally{
  ...
  async function gerarPDFRelatorioPremium(lista,dataIni,dataFim,btnRef){
    ...
    try{
      ...
      var _totM3=(lista||[]).reduce(function(s,m){return s+(parseFloat(m.medicao)||0);},0);
      var _totMud=(lista||[]).length;
      var _diasUnicos=[...new Set((lista||[]).map(function(m){return m.data;}).filter(Boolean))].length;
      var _comunidades=[...new Set((lista||[]).map(function(m){return (m.comunidade||'').trim();}).filter(Boolean))];
      var _fatBruto=_totM3*150 + _diasUnicos*1000; // fórmula Promorar
      var _fmtMoney=function(v){return 'R$ '+(v||0).toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2});};
      var _fmtNum=function(v){return (v||0).toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2});};

      // ─── PÁGINA 1: CAPA ─────────────────────────────────────
      // Fundo azul gradiente (3 camadas simulando gradiente)
      doc.setFillColor(30,58,138);doc.rect(0,0,pgW,pgH,'F'); // base
      doc.setFillColor(37,99,235);doc.rect(0,0,pgW,pgH*0.65,'F'); // meio
      doc.setFillColor(59,130,246);doc.rect(0,0,pgW,pgH*0.35,'F'); // topo

      // Logo
      try{var logoH=32;var logoW=logoH*1.41;doc.addImage(_LOGO_B64,'PNG',(pgW-logoW)/2,18,logoW,logoH);}catch(_){}

      // Faixa branca atrás do título
      doc.setFillColor(255,255,255);doc.rect(0,pgH*0.35-2,pgW,3,'F');

      // Título principal
      doc.setFontSize(28);doc.setFont('helvetica','bold');doc.setTextColor(255,255,255);
      doc.text('RELATÓRIO',pgW/2,pgH*0.45,{align:'center'});
      doc.setFontSize(20);doc.text('DE OPERAÇÕES',pgW/2,pgH*0.45+10,{align:'center'});

      // Subtítulo
      doc.setFontSize(11);doc.setFont('helvetica','normal');doc.setTextColor(219,234,254);
      doc.text('Contrato: PROMORAR',pgW/2,pgH*0.55,{align:'center'});

      // Box do período
      doc.setFillColor(255,255,255);doc.roundedRect(pgW/2-50,pgH*0.62,100,18,4,4,'F');
      doc.setFontSize(8);doc.setFont('helvetica','bold');doc.setTextColor(100,116,139);
      doc.text('PERÍODO',pgW/2,pgH*0.62+6,{align:'center'});
      doc.setFontSize(11);doc.setTextColor(30,58,138);
      doc.text(perStr,pgW/2,pgH*0.62+13,{align:'center'});

      // KPIs grandes na capa (4 cards horizontal)
      var kY=pgH*0.82;
      var cardW=(pgW-40)/4;
      var kpis=[
        {l:'MUDANÇAS',v:String(_totMud),sub:''},
        {l:'VOLUME',v:_fmtNum(_totM3),sub:'m³'},
        {l:'DIAS',v:String(_diasUnicos),sub:'operados'},
        {l:'COMUNIDADES',v:String(_comunidades.length),sub:'atendidas'}
      ];
      kpis.forEach(function(k,i){
        var x=20+i*cardW;
        doc.setFillColor(255,255,255,0.15);doc.roundedRect(x,kY,cardW-3,30,3,3,'F');
        doc.setFontSize(7);doc.setFont('helvetica','bold');doc.setTextColor(191,219,254);
        doc.text(k.l,x+cardW/2-1.5,kY+5,{align:'center'});
        doc.setFontSize(18);doc.setTextColor(255,255,255);
        doc.text(k.v,x+cardW/2-1.5,kY+18,{align:'center'});
        if(k.sub){doc.setFontSize(7);doc.setFont('helvetica','normal');doc.setTextColor(191,219,254);doc.text(k.sub,x+cardW/2-1.5,kY+24,{align:'center'});}
      });

      // Rodapé da capa
      doc.setFontSize(8);doc.setFont('helvetica','normal');doc.setTextColor(255,255,255);
      doc.text('Telemim Mudanças · Gerado em '+extractStr,pgW/2,pgH-6,{align:'center'});

      // ─── PÁGINA 2: RESUMO + GRÁFICO ──────────────────────────
      doc.addPage();
      doc.setTextColor(30,41,59);
      _addPDFHeader(doc,'RESUMO EXECUTIVO','Período: '+perStr);

      var Y=42;
      // 4 cards de KPI grandes
      var cWidth=(pgW-30)/2;var cHeight=28;
      var cards2=[
        {bg:[239,246,255],bd:[37,99,235],t:[30,58,138],l:'TOTAL DE MUDANÇAS',v:String(_totMud),u:'realizada'+(_totMud!==1?'s':'')},
        {bg:[240,253,244],bd:[22,163,74],t:[20,83,45],l:'VOLUME TOTAL',v:_fmtNum(_totM3),u:'m³ transportados'},
        {bg:[255,251,235],bd:[245,158,11],t:[120,53,15],l:'DIAS OPERADOS',v:String(_diasUnicos),u:'dias com mudança'},
        {bg:[252,231,243],bd:[219,39,119],t:[131,24,67],l:'COMUNIDADES',v:String(_comunidades.length),u:'atendidas'}
      ];
      cards2.forEach(function(c,i){
        var x=15+(i%2)*(cWidth+5);
        var y=Y+Math.floor(i/2)*(cHeight+5);
        doc.setFillColor(c.bg[0],c.bg[1],c.bg[2]);doc.roundedRect(x,y,cWidth,cHeight,3,3,'F');
        doc.setDrawColor(c.bd[0],c.bd[1],c.bd[2]);doc.setLineWidth(0.5);doc.roundedRect(x,y,cWidth,cHeight,3,3,'S');
        doc.setFontSize(7);doc.setFont('helvetica','bold');doc.setTextColor(c.t[0],c.t[1],c.t[2]);
        doc.text(c.l,x+4,y+6);
        doc.setFontSize(22);doc.text(c.v,x+4,y+18);
        doc.setFontSize(8);doc.setFont('helvetica','normal');doc.text(c.u,x+4,y+24);
      });
      Y+=cHeight*2+15;

      // Faturamento estimado
      doc.setFillColor(30,58,138);doc.roundedRect(15,Y,pgW-30,22,3,3,'F');
      doc.setFontSize(8);doc.setFont('helvetica','bold');doc.setTextColor(191,219,254);
      doc.text('FATURAMENTO BRUTO ESTIMADO (Medição × R$150 + Dias × R$1.000)',15+5,Y+6);
      doc.setFontSize(20);doc.setTextColor(255,255,255);
      doc.text(_fmtMoney(_fatBruto),15+5,Y+18);
      Y+=30;

      // Distribuição por comunidade (mini gráfico de barras)
      var porComun={};
      (lista||[]).forEach(function(m){var c=(m.comunidade||'Sem comunidade').trim();if(!porComun[c])porComun[c]={n:0,m3:0};porComun[c].n++;porComun[c].m3+=(parseFloat(m.medicao)||0);});
      var listComun=Object.entries(porComun).map(function(e){return {nome:e[0],n:e[1].n,m3:e[1].m3};}).sort(function(a,b){return b.m3-a.m3;}).slice(0,8);
      doc.setFontSize(10);doc.setFont('helvetica','bold');doc.setTextColor(30,41,59);
      doc.text('Distribuição por Comunidade (Top 8)',15,Y);
      Y+=6;
      var maxM3=Math.max.apply(null,listComun.map(function(x){return x.m3;}))||1;
      listComun.forEach(function(c){
        var barW=Math.round((c.m3/maxM3)*(pgW-100));
        doc.setFontSize(8);doc.setTextColor(71,85,105);
        doc.text((c.nome||'?').substring(0,30),15,Y+3.5);
        doc.setFillColor(37,99,235);doc.roundedRect(78,Y,barW,5,1,1,'F');
        doc.setFontSize(8);doc.setTextColor(30,58,138);
        doc.text(_fmtNum(c.m3)+' m³ ('+c.n+')',80+barW,Y+3.5);
        Y+=7;
      });
      ...
      doc.addPage();
      _addPDFHeader(doc,'DETALHAMENTO DE MUDANÇAS','Total: '+_totMud+'  |  Volume: '+_fmtNum(_totM3)+' m³');

      var rows=(lista||[]).sort(function(a,b){return (a.data||'').localeCompare(b.data||'');}).map(function(m){
        var d=m.data?m.data.split('-').reverse().join('/'):'—';
        return [d, m.horario||'—', m.nome||'—', (m.comunidade||'').substring(0,25), (m.origem||'').substring(0,30), (m.destino||'').substring(0,30), _fmtNum(parseFloat(m.medicao)||0)];
      });
      // Linha de total
      rows.push(['','','','','','TOTAL', _fmtNum(_totM3)]);

      doc.autoTable({
        startY:38,margin:{bottom:22,left:10,right:10},
        head:[['Data','Hora','Morador','Comunidade','Origem','Destino','m³']],
        body:rows,
        theme:'grid',
        styles:{fontSize:7,cellPadding:1.8,overflow:'linebreak',font:'helvetica',lineColor:[226,232,240]},
        headStyles:{fillColor:[30,58,138],textColor:[255,255,255],fontStyle:'bold',fontSize:8,halign:'center'},
        alternateRowStyles:{fillColor:[248,250,252]},
        columnStyles:{0:{cellWidth:18,halign:'center'},1:{cellWidth:12,halign:'center'},2:{cellWidth:42},3:{cellWidth:28},4:{cellWidth:38},5:{cellWidth:38},6:{cellWidth:15,halign:'right',fontStyle:'bold'}},
        didParseCell:function(data){
          // Linha de total estilizada
      ...
      doc.save('Relatorio_Premium_TELEMIM_'+_fmtDate(now)+'.pdf');
    }finally{
      if(btnRef){btnRef.disabled=false;btnRef.textContent='✨ Baixar Premium';}
    }
  }
  ...
  async function handleDeslocamento(ag, tipo){
    ...
    try{
      ...
      if(!r.ok) throw new Error("HTTP "+r.status);
      // IMPL 4 — aviso (não bloqueia): motorista sem rastreio GPS ativo
      var _rastreioWarn="";
      try{
        var _midDesl=tipo==="van"?ag.motorista_van_id:ag.motorista_caminhao_id;
        if(_midDesl){
          var _rs=await fetch(SUPA_URL+"/rest/v1/rpc/rastreio_status_link",{method:"POST",headers:Object.assign({},getH(),{"Content-Type":"application/json"}),body:JSON.stringify({p_mid:_midDesl})});
          if(_rs.ok){var _rsd=await _rs.json();var _row=Array.isArray(_rsd)?_rsd[0]:_rsd;if(_row&&_row.status!=="ativo")_rastreioWarn=" ⚠️ motorista sem rastreio GPS";}
        }
      }catch(e){}
      setSyncStatus("✅ Deslocamento registrado!"+_rastreioWarn);
      ...
      if(_novaMud){
        ...
        if(_eqConf.length>0){
          try{
            ...
            await fetch(SUPA_URL+"/rest/v1/contas_pagar",{method:"POST",headers:Object.assign({},getH(),{"Content-Type":"application/json","Prefer":"return=minimal"}),body:JSON.stringify({tipo:"PAGAR",categoria:"Pagamento de Equipe",descricao:_desc,valor:0,data:ag.data||_fmtDate(new Date()),os_id:_novaMud.id,agenda_id:ag.id,status:"pendente"})});
          }catch(e){}
        }
      }
      setAgenda(function(prev){return prev.filter(function(x){return x.id!==ag.id;});});
  ...
  // ── RASTREIO: bloco de instalação/config do OwnTracks (só se GPS inativo) ──
  // Retorna "" quando o motorista já está rastreando (não polui a mensagem).
  async function _owntracksBloco(mid){
    if(!mid)return "";
    try{
      var r=await fetch(SUPA_URL+"/rest/v1/rpc/rastreio_status_link",{method:"POST",headers:Object.assign({},getH(),{"Content-Type":"application/json"}),body:JSON.stringify({p_mid:mid})});
      if(!r.ok)return "";
      var d=await r.json();
      var row=Array.isArray(d)?d[0]:d;
      if(!row||!row.token)return "";
      if(row.status==="ativo")return "";
      var _cfg=SUPA_URL+"/functions/v1/owntracks-position?token="+row.token+"&config=1";
      return "\n━━━━━━━━━━━━━━\n⚠️ *ATIVE SEU RASTREAMENTO GPS*\nSeu GPS está "+(row.status==="inativo"?"inativo (+14 dias sem sinal)":"desativado")+". Antes de iniciar a mudança:\n\n1️⃣ Instale o app *OwnTracks*:\n📱 Android: https://play.google.com/store/apps/details?id=org.owntracks.android\n🍎 iPhone: https://apps.apple.com/app/owntracks/id692424691\n\n2️⃣ No OwnTracks abra *Preferences ▸ Configuration ▸ Load* e cole este link:\n"+_cfg+"\n\nPronto — o rastreio liga sozinho. 🚚📍";
    }catch(e){return "";}
  }
  async function reenviarMensagensMotorista(agId,motoristaId,tipo){
    ...
    try{
      ...
      if(_d2&&_d2.ok&&_d2.token){
        ...
        var _otBlocoR=await _owntracksBloco(_mot.id);
        enviarWA(_mot.contato,_msgRotaR+(_otBlocoR||""));
      }
...
{isMotorista&&mudancasFuturas.length>0&&(
        ...
        {isMotorista&&tab==="dashboard"&&(mudancasHoje.length>0||mudancasAmanha.length>0)&&(function(){
          ...
          var _hjStr=_fmtDate(_hj);var _amStr=_fmtDate(_am);
      ...
      {(function(){
        ...
        _allToday2.forEach(function(am){
          ...
          if(_vanTransit&&am.motorista_van_id){
            ...
            _activeVehicles.push({agId:am.id,motId:am.motorista_van_id,nome:_vm?_vm.nome:"Motorista",veiculo:"van",clienteNome:am.nome||"",origem:am.origem||"",destino:am.destino||""});
          }
          if(_camTransit&&am.motorista_caminhao_id){
            var _cm=listaUsuarios.find(function(u){return u.id===am.motorista_caminhao_id;});
            _activeVehicles.push({agId:am.id,motId:am.motorista_caminhao_id,nome:_cm?_cm.nome:"Motorista",veiculo:"cam",clienteNome:am.nome||"",origem:am.origem||"",destino:am.destino||""});
          }
        });
    ...
    return <div key={tipo+"_"+refId} style={{background:_statusBg(_st),borderRadius:10,padding:"12px 14px",marginBottom:8,border:"1px solid "+(_st==="pago"?"#bbf7d0":_st==="parcial"?"#fde68a":"#fecaca")}}>
      ...
      <div style={{display:"flex",gap:6}}>
        {_st!=="pago"&&<button onClick={function(){
          salvarPagamento({id:_pag?_pag.id:undefined,tipo:tipo,ref_id:refId,ref_nome:nome,periodo:_pagPeriodoKey,valor:valor,status:"pago",data_pagamento:_fmtDate(new Date()),metodo:"PIX",criado_em:_pag?_pag.criado_em:new Date().toISOString()});
        ...
        {_st!=="parcial"&&_st!=="pago"&&<button onClick={function(){
          salvarPagamento({id:_pag?_pag.id:undefined,tipo:tipo,ref_id:refId,ref_nome:nome,periodo:_pagPeriodoKey,valor:valor,status:"parcial",data_pagamento:_fmtDate(new Date()),metodo:"",criado_em:_pag?_pag.criado_em:new Date().toISOString()});
          ...
          <button onClick={function(){
            _ajListP.forEach(function(aj){
              ...
              if(!_pag||_pag.status!=="pago"){
                salvarPagamento({id:_pag?_pag.id:undefined,tipo:"ajudante",ref_id:aj.id,ref_nome:"👷 "+aj.nome,periodo:_pagPeriodoKey,valor:aj.total,status:"pago",data_pagamento:_fmtDate(new Date()),metodo:"PIX",criado_em:_pag?_pag.criado_em:new Date().toISOString()});
              }
            });
        ...
        {tab==="auditoria"&&isAdmin&&(function(){
          ...
          return(
            <div style={{paddingBottom:80}}>
              ...
              <div style={{display:"flex",background:"#f8fafc",borderBottom:"2px solid #e2e8f0"}}>
                {[{id:"saude",l:"🩺"},{id:"lixeira",l:"🗑️"},{id:"erros",l:"⚠️"},{id:"morador",l:"👤"},{id:"duplicatas",l:"👥"},{id:"historico",l:"📜"}].map(function(s){
                  return <button key={s.id} onClick={function(){setAuditSubTab(s.id);if(s.id==="lixeira"&&auditLixeira.length===0)loadAuditLixeira();else if(s.id==="erros"&&auditErros.length===0)loadAuditErros();else if(s.id==="saude"&&!auditSaude)loadAuditSaude();else if(s.id==="duplicatas"&&!auditDuplicatas)loadAuditDuplicatas();}} style={{flex:1,padding:"12px 2px",border:"none",cursor:"pointer",fontSize:18,fontWeight:auditSubTab===s.id?700:500,background:"transparent",borderBottom:auditSubTab===s.id?"3px solid #334155":"3px solid transparent",color:auditSubTab===s.id?"#334155":"#64748b"}} title={s.id}>{s.l}</button>;
                })}
              </div>
              ...
              {auditSubTab==="duplicatas"&&<div style={{padding:14}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                  <div><div style={{fontSize:13,fontWeight:800,color:"#1e293b"}}>👥 Ajudantes duplicados</div><div style={{fontSize:10,color:"#64748b"}}>Mesmo nome com IDs diferentes</div></div>
                  <button onClick={loadAuditDuplicatas} disabled={auditLoading} style={{padding:"6px 10px",borderRadius:8,border:"1.5px solid #334155",background:"#fff",color:"#334155",fontSize:11,fontWeight:700,cursor:"pointer"}}>{auditLoading?"⏳":"🔄"}</button>
                </div>
                {!auditDuplicatas?<div style={{textAlign:"center",padding:30,color:"#94a3b8",fontSize:13}}>{auditLoading?"⏳ Procurando...":"💡 Clique 🔄 pra começar"}</div>:
                auditDuplicatas.grupos.length===0?<div style={{textAlign:"center",padding:30,color:"#16a34a",fontSize:13}}>✨ Nenhuma duplicata encontrada</div>:
                <>
                  <div style={{background:"#fffbeb",border:"1px solid #fcd34d",borderRadius:10,padding:"10px 12px",marginBottom:12,fontSize:11,color:"#92400e"}}>⚠️ {auditDuplicatas.grupos.length} grupo(s) com duplicatas</div>
                  {auditDuplicatas.grupos.map(function(g){
                    return <div key={g.key} style={{background:"#fff",border:"1.5px solid #fcd34d",borderRadius:12,padding:"12px",marginBottom:10}}>
                      <div style={{fontWeight:800,fontSize:13,color:"#92400e",marginBottom:8}}>👤 {g.nome} <span style={{fontSize:10,color:"#64748b",fontWeight:500}}>({g.membros.length} cadastros)</span></div>
                      {g.membros.map(function(m){
                        return <div key={m.id} style={{background:m.ativo?"#f0fdf4":"#f8fafc",border:"1px solid "+(m.ativo?"#bbf7d0":"#e2e8f0"),borderRadius:8,padding:"8px 10px",marginBottom:6,fontSize:11}}>
                          <div style={{fontWeight:700,color:m.ativo?"#15803d":"#94a3b8"}}>id={m.id} {m.ativo?"✅ ativo":"❌ inativo"}</div>
                          <div style={{fontSize:10,color:"#64748b",marginTop:2}}>📞 {m.telefone||"sem tel"} · 💳 {m.pix||"sem pix"}</div>
                          <div style={{fontSize:10,color:"#1e40af",marginTop:2}}>📅 {m._dias90||0} dia(s) últ. 90d</div>
                        </div>;
                      })}
                      <button onClick={function(){
                        var _ativos=g.membros.filter(function(m){return m.ativo;});
                        var _pool=_ativos.length>0?_ativos:g.membros;
                        var _target=_pool.reduce(function(a,b){return (a._dias90||0)>=(b._dias90||0)?a:b;});
                        var _sources=g.membros.filter(function(m){return m.id!==_target.id;});
                        setMergeAjudantes({nome:g.nome,target:_target,sources:_sources});
                      }} style={{width:"100%",padding:"8px",borderRadius:8,border:"none",background:"#1e40af",color:"#fff",fontSize:12,fontWeight:800,cursor:"pointer",marginTop:6}}>🔀 Mesclar em 1 ajudante</button>
                    </div>;
                  })}
                </>}
              </div>}
              {mergeAjudantes&&<div onClick={function(){setMergeAjudantes(null);}} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.55)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
                <div onClick={function(e){e.stopPropagation();}} style={{background:"#fff",borderRadius:20,padding:"22px",maxWidth:400,width:"100%",boxShadow:"0 8px 40px rgba(0,0,0,0.25)",maxHeight:"80vh",overflow:"auto"}}>
                  <div style={{fontWeight:900,fontSize:15,color:"#1e40af",marginBottom:12}}>🔀 Confirmar merge</div>
                  <div style={{background:"#f0fdf4",border:"1.5px solid #bbf7d0",borderRadius:10,padding:"10px 12px",marginBottom:8}}>
                    <div style={{fontSize:10,fontWeight:700,color:"#15803d"}}>MANTÉM (target)</div>
                    <div style={{fontWeight:700,fontSize:13,color:"#14532d"}}>id={mergeAjudantes.target.id} · {mergeAjudantes.target.nome}</div>
                    <div style={{fontSize:11,color:"#64748b",marginTop:2}}>📞 {mergeAjudantes.target.telefone||"—"} · 💳 {mergeAjudantes.target.pix||"—"} · 📅 {mergeAjudantes.target._dias90||0} dias</div>
                  </div>
                  <div style={{fontSize:10,color:"#94a3b8",textAlign:"center",margin:"6px 0"}}>↓ mesclar nele ↓</div>
                  {mergeAjudantes.sources.map(function(s){return <div key={s.id} style={{background:"#fef2f2",border:"1.5px solid #fecaca",borderRadius:10,padding:"10px 12px",marginBottom:6}}>
                    <div style={{fontSize:10,fontWeight:700,color:"#dc2626"}}>SERÁ DESATIVADO</div>
                    <div style={{fontWeight:700,fontSize:13,color:"#7f1d1d"}}>id={s.id} · {s.nome}</div>
                    <div style={{fontSize:11,color:"#64748b",marginTop:2}}>📞 {s.telefone||"—"} · 💳 {s.pix||"—"} · 📅 {s._dias90||0} dias</div>
                  </div>;})}
                  <div style={{background:"#fffbeb",border:"1px solid #fcd34d",borderRadius:8,padding:"8px 10px",fontSize:10,color:"#92400e",marginTop:8,marginBottom:14}}>⚠️ Todos os dias dos sources serão atribuídos ao target.</div>
                  <div style={{display:"flex",gap:8}}>
                    <button onClick={function(){setMergeAjudantes(null);}} style={{flex:1,padding:"12px 0",borderRadius:12,border:"1.5px solid #e2e8f0",background:"#f8fafc",color:"#64748b",fontWeight:800,fontSize:13,cursor:"pointer"}}>Cancelar</button>
                    <button onClick={executarMerge} style={{flex:2,padding:"12px 0",borderRadius:12,border:"none",background:"#1e40af",color:"#fff",fontWeight:900,fontSize:13,cursor:"pointer"}}>🔀 Sim, mesclar</button>
                  </div>
                </div>
              </div>}
      ...
      {showPwaModal&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",backdropFilter:"blur(6px)",zIndex:2500,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={function(){setShowPwaModal(false);}}>
          <div style={{background:"#fff",borderRadius:20,padding:"28px 22px",width:"100%",maxWidth:360,textAlign:"center"}} onClick={function(e){e.stopPropagation();}}>
            ...
            <div style={{fontSize:18,fontWeight:900,color:"#1e293b",marginBottom:8}}>Ative seu rastreamento</div>
            <div style={{fontSize:13,color:"#475569",marginBottom:16,lineHeight:1.5}}>Para o GPS funcionar com a <b>tela bloqueada</b>, instale o app <b>OwnTracks</b> no seu celular.</div>
            <div style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:12,padding:"12px 14px",marginBottom:16,textAlign:"left"}}>
              <div style={{fontSize:12,fontWeight:700,color:"#166534",marginBottom:8}}>Depois de instalar:</div>
              <div style={{fontSize:12,color:"#15803d",lineHeight:1.7}}>
                Toque no <b>link de configuração</b> que você recebeu no WhatsApp da mudança (em <i>Preferences ▸ Configuration ▸ Load</i>). O rastreio liga sozinho.
              </div>
            </div>
            <div style={{display:"flex",gap:8,marginBottom:12}}>
              <button onClick={function(){window.open("https://play.google.com/store/apps/details?id=org.owntracks.android","_blank");}} style={{flex:1,padding:"12px 0",borderRadius:12,border:"none",background:"#16a34a",color:"#fff",fontWeight:800,fontSize:13,cursor:"pointer"}}>Android</button>
              <button onClick={function(){window.open("https://apps.apple.com/app/owntracks/id692424691","_blank");}} style={{flex:1,padding:"12px 0",borderRadius:12,border:"none",background:"#1e293b",color:"#fff",fontWeight:800,fontSize:13,cursor:"pointer"}}>iPhone</button>
            </div>
            <button onClick={function(){localStorage.setItem("tmim_rastreio_skip","1");setShowPwaModal(false);}} style={{width:"100%",padding:"12px 0",borderRadius:10,border:"1px solid #e2e8f0",background:"#f8fafc",color:"#334155",fontWeight:700,fontSize:13,cursor:"pointer"}}>Fechar</button>
          </div>
        </div>
      )}
    ...
    {reagendarModal&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={function(){setReagendarModal(null);}}><div style={{background:"#fff",borderRadius:20,padding:"28px 24px",maxWidth:380,width:"100%",boxShadow:"0 8px 40px rgba(0,0,0,0.25)"}} onClick={function(e){e.stopPropagation();}}>
      ...
      <div style={{display:"flex",gap:10}}>
        ...
        {toastMsg&&<div style={{position:"fixed",top:16,left:16,right:16,zIndex:10000,background:"#1e293b",color:"#fff",padding:"12px 16px",borderRadius:12,boxShadow:"0 8px 28px rgba(0,0,0,0.3)",display:"flex",alignItems:"center",gap:10,cursor:"pointer"}} onClick={function(){setToastMsg("");setTab("financeiro");}}>
          ...
          <div style={{fontSize:10,color:"#94a3b8",fontWeight:700}}>TOQUE</div>
        </div>}
