    ...
    <div style={{minHeight:"100vh",background:COLORS.bg,fontFamily:"'Segoe UI',system-ui,sans-serif",color:COLORS.text,paddingBottom:50}}>
      ...
      {editMud&&podeEditar&&(
        ...
        <div style={{marginTop:24,padding:20,background:'#fff',borderRadius:16,boxShadow:'0 2px 12px rgba(0,0,0,0.06)',border:'1px solid #e2e8f0'}}>
          ...
          <div style={{background:COLORS.bg,borderRadius:10,padding:12,marginBottom:16,fontSize:12,color:COLORS.muted}}><strong>Ex. 3 mud:</strong> R$ {(cfgEdit.van1a+2*cfgEdit.vanAdd).toFixed(2)} | <strong>4 mud:</strong> R$ {(cfgEdit.van1a+3*cfgEdit.vanAdd).toFixed(2)}</div>
          {cfgSaved&&<div style={{color:'#16a34a',fontSize:12,fontWeight:700,marginBottom:8}}>✅ Salvo!</div>}
          <button onClick={async function(){const cc=[{k:'van_1a_mudanca',v:String(cfgEdit.van1a)},{k:'van_adicional',v:String(cfgEdit.vanAdd)},{k:'ajudante_1a_mudanca',v:String(cfgEdit.aj1a)},{k:'ajudante_adicional',v:String(cfgEdit.ajAdd)},{k:'data_inicio_regra',v:cfgEdit.dataInicioRegra},{k:'imposto_pct',v:String(cfgEdit.imposto)}];for(const x of cc){await fetch(SUPA_URL+'/rest/v1/configuracoes?chave=eq.'+x.k,{method:'PATCH',headers:{'apikey':SUPA_KEY,'Authorization':'Bearer '+(usuario?.token||SUPA_KEY),'Content-Type':'application/json'},body:JSON.stringify({valor:x.v})});}RULES.van1a=cfgEdit.van1a;RULES.vanAdd=cfgEdit.vanAdd;RULES.aj1a=cfgEdit.aj1a;RULES.ajAdd=cfgEdit.ajAdd;RULES.imposto=cfgEdit.imposto/100;RULES.dataInicioRegra=cfgEdit.dataInicioRegra;setCfgSaved(true);setTimeout(()=>setCfgSaved(false),3000);}} style={{width:'100%',background:COLORS.accent,color:'#fff',border:'none',borderRadius:12,padding:'12px 0',fontWeight:700,fontSize:15,cursor:'pointer'}}>💾 Salvar Configurações</button>
        </div>
        </div>
      )}
