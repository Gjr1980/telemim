...
export default function App(){
  ...
  if(!usuario)useEffect(function(){var prev=document.getElementById("cdm");if(prev){prev.style.display=confirmDelete?"flex":"none";return;}if(!confirmDelete)return;var ov=document.createElement("div");ov.id="cdm";ov.style.cssText="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.6);z-index:9999;display:flex;align-items:center;justify-content:center;padding:16px";var box=document.createElement("div");box.style.cssText="background:#fff;border-radius:20px;padding:28px 24px;max-width:320px;width:100%;text-align:center";var t=document.createElement("p");t.style.cssText="font-size:36px;margin:0 0 8px";t.textContent="⚠️";var h=document.createElement("p");h.style.cssText="font-weight:800;font-size:15px;margin:0 0 8px;color:#1e293b";h.textContent="Tem a certeza?";var s=document.createElement("p");s.style.cssText="font-size:13px;color:#64748b;margin:0 0 18px";s.textContent="Apagar "+confirmDelete.nome+"?";var row=document.createElement("div");row.style.cssText="display:flex;gap:10px";var bNo=document.createElement("button");bNo.style.cssText="flex:1;padding:11px 0;border-radius:12px;border:1.5px solid #e2e8f0;background:#f8fafc;color:#64748b;font-weight:700;font-size:13px;cursor:pointer";bNo.textContent="Cancelar";bNo.onclick=function(){setConfirmDelete(null);};var bYes=document.createElement("button");bYes.style.cssText="flex:1;padding:11px 0;border-radius:12px;border:none;background:#ef4444;color:#fff;font-weight:800;font-size:13px;cursor:pointer";bYes.textContent="Sim, Apagar";bYes.onclick=function(){if(confirmDelete.tipo==="mud")handleDelMud(confirmDelete.id);else handleDelAg(confirmDelete.id);setConfirmDelete(null);};row.appendChild(bNo);row.appendChild(bYes);box.appendChild(t);box.appendChild(h);box.appendChild(s);box.appendChild(row);ov.appendChild(box);ov.onclick=function(ev){if(ev.target===ov)setConfirmDelete(null);};document.body.appendChild(ov);return function(){var x=document.getElementById("cdm");if(x)x.parentNode&&x.parentNode.removeChild(x);};},[confirmDelete]);
  function _showModal(id,nome,tipo){var ov=document.createElement("div");ov.id="cdm";ov.style.cssText="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.6);z-index:9999;display:flex;align-items:center;justify-content:center;padding:16px";function _close(){var x=document.getElementById("cdm");if(x)x.parentNode.removeChild(x);}var box=document.createElement("div");box.style.cssText="background:#fff;border-radius:20px;padding:28px 24px;max-width:320px;width:100%;text-align:center";var t=document.createElement("p");t.style.cssText="font-size:36px;margin:0 0 8px";t.textContent="⚠️";var h=document.createElement("p");h.style.cssText="font-weight:800;font-size:15px;margin:0 0 6px;color:#1e293b";h.textContent="Tem a certeza?";var s=document.createElement("p");s.style.cssText="font-size:13px;color:#64748b;margin:0 0 18px";s.textContent="Apagar "+nome+"?";var row=document.createElement("div");row.style.cssText="display:flex;gap:10px";var bNo=document.createElement("button");bNo.style.cssText="flex:1;padding:11px 0;border-radius:12px;border:1.5px solid #e2e8f0;background:#f8fafc;color:#64748b;font-weight:700;font-size:13px;cursor:pointer";bNo.textContent="Cancelar";bNo.onclick=_close;var bYes=document.createElement("button");bYes.style.cssText="flex:1;padding:11px 0;border-radius:12px;border:none;background:#ef4444;color:#fff;font-weight:800;font-size:13px;cursor:pointer";bYes.textContent="🗑️ Apagar";bYes.onclick=function(){_close();if(tipo==="mud")handleDelMud(id);else handleDelAg(id);};row.appendChild(bNo);row.appendChild(bYes);box.appendChild(t);box.appendChild(h);box.appendChild(s);box.appendChild(row);ov.appendChild(box);ov.onclick=function(e){if(e.target===ov)_close();};document.body.appendChild(ov);}
              ...
              <Card key={m.id} style={{marginBottom:8,padding:"13px 15px"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  ...
                  <div style={{display:"flex",gap:5,alignItems:"center",marginLeft:8}}>
                    ...
                    {(usuario&&(usuario.perfil==="admin"||usuario.perfil==="telemim"))&&<button onClick={function(e){e.stopPropagation();_showModal(m.id,m.nome,"mud");}} style={btnRed}>✕</button>}
                  </div>
                </div>
                  ...
                  <div id={"move-card-"+a.id}><Card key={a.id} style={{marginBottom:9,padding:"14px 16px",border:`1.5px solid ${statusColor[a.status]||COLORS.cardBorder}33`}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                      ...
                      <div style={{display:"flex",flexDirection:"column",gap:5,marginLeft:9}}>
                        ...
                        {(usuario&&(usuario.perfil==="admin"||usuario.perfil==="telemim"))&&<button onClick={function(e){e.stopPropagation();_showModal(a.id,a.nome,"ag");}} style={btnRed}>✕</button>}
                      </div>
                    </div>
