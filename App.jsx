            ...
            return(<>
              ...
              <div style={{padding:'0 12px 10px',background:'#f8fafc'}}>
                <div style={{background:'#fff',borderRadius:14,padding:16,boxShadow:'0 2px 8px rgba(0,0,0,0.06)'}}>
                  ...
                  {(function(){var hj2=new Date();var dow=hj2.getDay();var sg=new Date(hj2);sg.setDate(hj2.getDate()-dow+(dow===0?-6:1));return Array.from({length:7},function(_,i){var dd=new Date(sg);dd.setDate(sg.getDate()+i);return dd.toISOString().slice(0,10);}).map(function(dia){var mD=list.filter(function(m){return m.data===dia;});var cD=custos.find(function(cd){return cd.data===dia;});var nD=mD.length;var fvD=nD>0?RULES.van1a+(nD-1)*RULES.vanAdd:0;var fmD=mD.reduce(function(s,m){return s+(parseFloat(m.medicao)||0);},0)*RULES.medicaoPorM3;var brD=fvD+fmD;var lqD=brD*(1-RULES.imposto);var ajD=cD?cD.ajudantes||0:0;var cAD=ajD>0?(RULES.aj1a+(nD>0?nD-1:0)*RULES.ajAdd)*ajD:0;var cuD=nD>0?RULES.vanCusto+cAD+(parseFloat(cD&&cD.custo_almoco)||0):0;var luD=nD>0?lqD-cuD:0;var isHj=dia===hj2.toISOString().slice(0,10);var nm=['Dom','Seg','Ter','Qua','Qui','Sex','Sáb'][new Date(dia+'T12:00:00').getDay()];var df=dia.slice(8)+'/'+dia.slice(5,7);return(<div key={dia} style={{display:'flex',alignItems:'center',padding:'10px 12px',borderRadius:10,marginBottom:6,background:isHj?'#eff6ff':'#f8fafc',border:isHj?'1.5px solid #3b82f6':'1px solid #e2e8f0'}}><div style={{width:42,flexShrink:0}}><div style={{fontSize:10,color:isHj?'#3b82f6':'#94a3b8',fontWeight:700}}>{nm}</div><div style={{fontSize:13,fontWeight:800,color:isHj?'#1e40af':'#475569'}}>{df}</div></div>{nD>0?(<div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'space-between'}}><div><span style={{background:'#dbeafe',color:'#1e40af',borderRadius:6,padding:'2px 6px',fontSize:11,fontWeight:700,marginRight:4}}>{nD} mud.</span>{ajD>0&&<span style={{background:'#fef3c7',color:'#92400e',borderRadius:6,padding:'2px 5px',fontSize:10}}>{ajD} aj.</span>}</div><div style={{textAlign:'right'}}><div style={{fontSize:13,fontWeight:700,color:'#047857'}}>R$ {Math.round(luD).toLocaleString('pt-BR')}</div><div style={{fontSize:9,color:'#94a3b8'}}>bruto {fN(brD)}</div></div></div>):(<div style={{flex:1,textAlign:'center',fontSize:11,color:'#cbd5e1'}}>sem mudanças</div>)}</div>);});})()}
                </div>
              </div>
