          var _hj3=new Date();var _am3=(periodoFin==="mes_ant"?(function(){var d=new Date();d.setDate(1);d.setMonth(d.getMonth()-1);return d.toISOString().slice(0,7);})():_hj3.toISOString().slice(0,7));var _mm3=(mudancas||[]).filter(function(m){return !m.deleted_at&&m.data&&m.data.slice(0,7)===_am3;});var _dd3=[...new Set(_mm3.map(function(m){return m.data;}))].sort(function(a,b){return b.localeCompare(a);});
          return(
            <div>
            <div style={{margin:"0 12px 16px",background:"linear-gradient(135deg,#1e293b,#1e3a8a)",borderRadius:18,padding:"20px 18px 18px",boxShadow:"0 8px 32px rgba(30,41,59,0.25)"}}>(usuario.token.split(".")[1]));const ok=pl.exp*1000>Date.now()+30000;if(ok)return usuario.token;if(!usuario.refresh_token)return usuario.token;const res=await fetch(SUPA_URL+"/auth/v1/token?grant_type=refresh_token",{method:"POST",headers:{"apikey":SUPA_KEY,"Content-Type":"application/json"},body:JSON.stringify({refresh_token:usuario.refresh_token})});const d=await res.json();if(d.access_token){const saved=JSON.parse(localStorage.getItem("tmim_u")||"{}");saved.token=d.access_token;if(d.refresh_token)saved.refresh_token=d.refresh_token;localStorage.setItem("tmim_u",JSON.stringify(saved));return d.access_token;}}catch(e){}return usuario.token;};
...
{mudancasAmanha.length>0&&(
        ...
        <div style={{display:isMotorista&&abaMotorista!=='registros'?'none':undefined}}>{tab==="registros_mot"&&isMotorista&&(function(){
          ...
          return (
            ...
            {_mm3.length>0&&<div style={{padding:"0 12px 16px",background:"#f8fafc"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}><div style={{fontWeight:800,fontSize:15,color:"#1e293b"}}>📋 Mudanças do Mês</div><span style={{background:"#e0e7ff",color:"#3730a3",borderRadius:20,padding:"4px 12px",fontSize:12,fontWeight:700}}>{_mm3.length} total</span></div>{_dd3.map(function(dia){var _md3=_mm3.filter(function(m){return m.data===dia;});var _df3=dia.slice(8)+"/"+dia.slice(5,7)+"/"+dia.slice(0,4);var _ih3=dia===_hj3.toISOString().slice(0,10);return(<div key={dia} style={{background:"#fff",borderRadius:12,padding:"14px 16px",marginBottom:10,border:_ih3?"1.5px solid #3b82f6":"1px solid #e2e8f0"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><div style={{fontWeight:700,fontSize:14,color:_ih3?"#1e40af":"#1e293b"}}>{_df3}</div><span style={{background:"#e0e7ff",color:"#3730a3",borderRadius:20,padding:"3px 10px",fontSize:12,fontWeight:700}}>{_md3.length} mud.</span></div>{_md3.map(function(m,_i3){return(<div key={_i3} style={{display:"flex",alignItems:"center",padding:"7px 0",borderTop:_i3>0?"1px solid #f1f5f9":"none"}}><div style={{width:7,height:7,borderRadius:"50%",background:"#f59e0b",marginRight:10,flexShrink:0}}></div><div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:"#334155"}}>{m.nome}</div><div style={{fontSize:11,color:"#94a3b8"}}>{m.comunidade||""}</div></div></div>);})}</div>);})}</div></div>            </div>
          );
        })()}
