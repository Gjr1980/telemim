              ...
              <Card key={m.id} style={{marginBottom:10,padding:0,overflow:"hidden"}}>
                ...
                <div style={{padding:"10px 16px 8px",fontSize:12,color:"#475569"}}>
                  ...
                  <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:4}}>
                    ...
                    <span>{m.approved_by_promorar?<b style={{color:"#16a34a"}}>✅ Promorar</b>:<span style={{color:"#9ca3af"}}>⏳ Promorar</span>}</span>
                  </div>
                  {(function(){var _vanMot=null,_camMot=null;if(m.motorista_van_id){var _fv=listaUsuarios.find(function(u){return u.id===m.motorista_van_id;});if(_fv)_vanMot=_fv.nome;}if(m.motorista_caminhao_id){var _fc=listaUsuarios.find(function(u){return u.id===m.motorista_caminhao_id;});if(_fc)_camMot=_fc.nome;}return <>{m.motorista_van_id&&<div style={{marginBottom:2}}>🚐 Van: <b style={{color:"#2563eb"}}>{_vanMot||m.motorista_van_id.slice(0,8)}</b></div>}{m.motorista_caminhao_id&&<div style={{marginBottom:2}}>🚚 Caminhão: <b style={{color:"#7c3aed"}}>{_camMot||m.motorista_caminhao_id.slice(0,8)}</b></div>}</>;})()}
