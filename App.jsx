...
{(tab==="relatorio"||tab==="semana")&&isAdmin&&(()=>{const hoje=new Date();const anoMes=hoje.toISOString().substring(0,7);const mudMes=mudancas.filter(m=>m.data&&m.data.startsWith(anoMes));const datasMes=[...new Set(mudMes.map(m=>m.data))].sort();const diasMes=datasMes.map(data=>{const mds=mudMes.filter(m=>m.data===data);const vd=mds.length;const m3=mds.reduce((s,m)=>s+(parseFloat(m.medicao)||0),0);const cd=custosDiarios.find(c=>c.data===data)||{};const aj=parseInt(cd.ajudantes)||0;const fatM=m3*RULES.medicaoPorM3;const fatV=vd>0?RULES.van1a+(vd-1)*RULES.vanAdd:0;const bruto=fatM+fatV;const imp=bruto*RULES.imposto;const liq=bruto-imp;const cAj=aj>0?(RULES.aj1a+(vd>0?vd-1:0)*RULES.ajAdd)*aj:0;const custo=RULES.vanCusto+cAj+(parseFloat(cd.custo_almoco)||0);const lucro=liq-custo;return {data,vd,m3,aj,bruto,imp,liq,custo,lucro};});const totBruto=diasMes.reduce((s,d)=>s+d.bruto,0);const totLiq=diasMes.reduce((s,d)=>s+d.liq,0);const totCusto=diasMes.reduce((s,d)=>s+d.custo,0);const totLucro=diasMes.reduce((s,d)=>s+d.lucro,0);const totMud=diasMes.reduce((s,d)=>s+d.vd,0);const totM3=diasMes.reduce((s,d)=>s+d.m3,0);const margem=totLiq>0?((totLucro/totLiq)*100).toFixed(1):"0";const hoje7=new Date();const dias7=Array.from({length:7},(_,i)=>{const d=new Date(hoje7);d.setDate(d.getDate()-6+i);return d.toISOString().substring(0,10);});const diasSem=dias7.map(data=>{const mds=mudancas.filter(m=>m.data===data);const vd=mds.length;const m3=mds.reduce((s,m)=>s+(parseFloat(m.medicao)||0),0);const cd=custosDiarios.find(c=>c.data===data)||{};const aj=parseInt(cd.ajudantes)||0;const fatV=vd>0?RULES.van1a+(vd-1)*RULES.vanAdd:0;const bruto=m3*RULES.medicaoPorM3+fatV;const liq=bruto*(1-RULES.imposto);const cAj=aj>0?(RULES.aj1a+(vd>0?vd-1:0)*RULES.ajAdd)*aj:0;const custo=RULES.vanCusto+cAj+(parseFloat(cd.custo_almoco)||0);const lucro=liq-custo;const isHoje=data===hoje7.toISOString().substring(0,10);const diaNome=["Dom","Seg","Ter","Qua","Qui","Sex","Sab"][new Date(data+"T12:00:00").getDay()];const diaNum=data.substring(8,10);return {data,vd,m3,aj,bruto,liq,custo,lucro,isHoje,diaNome,diaNum};});const semBruto=diasSem.reduce((s,d)=>s+d.bruto,0);const semLucro=diasSem.reduce((s,d)=>s+d.lucro,0);const fmt=v=>"R$ "+parseFloat(v||0).toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2});const mesNome=hoje.toLocaleString("pt-BR",{month:"long",year:"numeric"});const kpis=[{label:"Fat. Bruto",val:fmt(totBruto),icon:"📦",cor:"#1e40af"},{label:"Rec. Liquida",val:fmt(totLiq),icon:"💵",cor:"#0891b2"},{label:"Custo Total",val:fmt(totCusto),icon:"🔧",cor:"#dc2626"},{label:"Lucro",val:fmt(totLucro),icon:"🏆",cor:totLucro>=0?"#16a34a":"#dc2626"}];const resumo=[{label:"Mudancas",val:String(totMud)},{label:"M3 Total",val:totM3.toFixed(0)+"m3"},{label:"Margem",val:margem+"%"},{label:"Imposto",val:fmt(totBruto*RULES.imposto)}];return(<div style={{padding:"0 0 100px 0"}}><div style={{background:"linear-gradient(135deg,#1e40af,#3b82f6)",padding:"20px 16px 28px",color:"#fff"}}><div style={{fontSize:13,opacity:0.85,marginBottom:4}}>{"💰 Financeiro"}</div><div style={{fontSize:18,fontWeight:800,textTransform:"capitalize"}}>{mesNome}</div></div><div style={{padding:"0 12px",marginTop:-16}}><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>{kpis.map((k,i)=>(<div key={i} style={{background:"#fff",borderRadius:14,padding:"14px 12px",boxShadow:"0 2px 8px rgba(0,0,0,0.08)",borderTop:"3px solid "+k.cor}}><div style={{fontSize:11,color:"#64748b",fontWeight:600,marginBottom:4}}>{k.icon+" "+k.label}</div><div style={{fontSize:16,fontWeight:800,color:k.cor}}>{k.val}</div></div>))}</div><div style={{background:"#f8fafc",borderRadius:14,padding:"12px 14px",marginBottom:14,display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:8,border:"1px solid #e2e8f0"}}>{resumo.map((s,i)=>(<div key={i} style={{textAlign:"center",minWidth:70}}><div style={{fontSize:15,fontWeight:800,color:"#1e293b"}}>{s.val}</div><div style={{fontSize:10,color:"#94a3b8",fontWeight:600}}>{s.label}</div></div>))}</div><div style={{background:"#fff",borderRadius:14,padding:"14px 12px",marginBottom:14,boxShadow:"0 2px 8px rgba(0,0,0,0.06)"}}><div style={{fontWeight:800,fontSize:14,color:"#1e293b",marginBottom:2}}>{"📅 Semana Atual"}</div><div style={{fontSize:11,color:"#64748b",marginBottom:10}}>{"Fat. "+fmt(semBruto)+" · Lucro "+fmt(semLucro)}</div><div style={{display:"flex",gap:6,justifyContent:"space-between"}}>{diasSem.map((d,i)=>(<div key={i} style={{flex:1,borderRadius:10,padding:"8px 4px",textAlign:"center",background:d.isHoje?"#1e40af":d.vd>0?"#eff6ff":"#f8fafc",border:d.isHoje?"2px solid #1e40af":"2px solid transparent"}}><div style={{fontSize:10,fontWeight:700,color:d.isHoje?"#fff":"#64748b"}}>{d.diaNome}</div><div style={{fontSize:12,fontWeight:800,color:d.isHoje?"#fff":"#1e293b",margin:"2px 0"}}>{d.diaNum}</div>{d.vd>0?<div style={{fontSize:9,color:d.isHoje?"#bfdbfe":"#3b82f6",fontWeight:700}}>{String(d.vd)+"🚛"}</div>:<div style={{fontSize:9,color:d.isHoje?"#93c5fd":"#cbd5e1"}}>{"—"}</div>}{d.lucro!==0&&<div style={{fontSize:8,color:d.isHoje?"#a5f3fc":d.lucro>0?"#16a34a":"#dc2626",fontWeight:700,marginTop:1}}>{(d.lucro>0?"+":"")+(d.lucro/1000).toFixed(1)+"k"}</div>}</div>))}</div></div><div style={{background:"#fff",borderRadius:14,padding:"14px 12px",boxShadow:"0 2px 8px rgba(0,0,0,0.06)"}}><div style={{fontWeight:800,fontSize:14,color:"#1e293b",marginBottom:12}}>{"📊 Historico do Mes"}</div>{diasMes.length===0&&<div style={{textAlign:"center",color:"#94a3b8",padding:"20px 0",fontSize:13}}>{"Nenhuma mudanca este mes"}</div>}{diasMes.map((d,i)=>{const dt=new Date(d.data+"T12:00:00");const dNome=["Dom","Seg","Ter","Qua","Qui","Sex","Sab"][dt.getDay()];const dFmt=dNome+" "+d.data.substring(8,10)+"/"+d.data.substring(5,7);return(<div key={i} style={{borderBottom:i<diasMes.length-1?"1px solid #f1f5f9":"none",paddingBottom:12,marginBottom:12}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><div style={{fontWeight:700,fontSize:13,color:"#1e293b"}}>{dFmt}</div><div style={{fontWeight:800,fontSize:14,color:d.lucro>=0?"#16a34a":"#dc2626"}}>{fmt(d.lucro)}</div></div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}><span style={{background:"#eff6ff",color:"#1d4ed8",borderRadius:8,padding:"2px 8px",fontSize:11,fontWeight:600}}>{String(d.vd)+"🚛 · "+d.m3+"m³"}</span>{d.aj>0&&<span style={{background:"#f0fdf4",color:"#166534",borderRadius:8,padding:"2px 8px",fontSize:11,fontWeight:600}}>{String(d.aj)+"👷"}</span>}<span style={{background:"#fef9c3",color:"#92400e",borderRadius:8,padding:"2px 8px",fontSize:11,fontWeight:600}}>{"Liq "+fmt(d.liq)}</span><span style={{background:"#fef2f2",color:"#991b1b",borderRadius:8,padding:"2px 8px",fontSize:11,fontWeight:600}}>{"Custo "+fmt(d.custo)}</span></div></div>);})}</div></div></div>);})()}
