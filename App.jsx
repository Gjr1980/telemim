...
{tab==="financeiro"&&<div style={{padding:"8px 12px 12px",background:"#f8fafc"}}><button onClick={function(){window.print();}} style={{width:"100%",padding:"12px",borderRadius:12,border:"none",background:"linear-gradient(135deg,#1e40af,#1e293b)",color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>📄 Exportar PDF</button></div>}
      ...
      {tab==="lista"&&(
          <div>
            <div style={{padding:'8px 12px 0'}}><div style={{display:'flex',gap:6,marginBottom:8}}><button onClick={()=>{setFiltroMes('semana');setFiltroDataIni('');setFiltroDataFim('');}} style={{padding:'6px 14px',borderRadius:16,border:'none',cursor:'pointer',fontSize:12,fontWeight:600,background:filtroMes==='semana'&&!filtroDataIni?'#1e40af':'#e2e8f0',color:filtroMes==='semana'&&!filtroDataIni?'#fff':'#475569'}}>Semana</button><button onClick={()=>{setFiltroMes('mes_atual');setFiltroDataIni('');setFiltroDataFim('');}} style={{padding:'6px 14px',borderRadius:16,border:'none',cursor:'pointer',fontSize:12,fontWeight:600,background:filtroMes==='mes_atual'&&!filtroDataIni?'#1e40af':'#e2e8f0',color:filtroMes==='mes_atual'&&!filtroDataIni?'#fff':'#475569'}}>Mês Atual</button><button onClick={()=>{setFiltroMes('');setFiltroDataIni('');setFiltroDataFim('');}} style={{padding:'6px 14px',borderRadius:16,border:'none',cursor:'pointer',fontSize:12,fontWeight:600,background:filtroMes===''&&!filtroDataIni?'#1e40af':'#e2e8f0',color:filtroMes===''&&!filtroDataIni?'#fff':'#475569'}}>Todos</button></div><div style={{display:'flex',gap:6,alignItems:'center',marginBottom:4}}><input type='date' value={filtroDataIni} onChange={e=>{setFiltroDataIni(e.target.value);setFiltroMes('datas');}} style={{flex:1,padding:'5px 8px',borderRadius:8,border:'1px solid #e2e8f0',fontSize:12,color:'#334155'}} /><span style={{fontSize:11,color:'#94a3b8',whiteSpace:'nowrap'}}>até</span><input type='date' value={filtroDataFim} onChange={e=>{setFiltroDataFim(e.target.value);setFiltroMes('datas');}} style={{flex:1,padding:'5px 8px',borderRadius:8,border:'1px solid #e2e8f0',fontSize:12,color:'#334155'}} /></div></div><div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:6,paddingBottom:6,borderBottom:'1px solid #e2e8f0'}}><span style={{fontSize:11,color:'#64748b',alignSelf:'center',fontWeight:700,marginRight:2}}>Confirmação:</span><button onClick={()=>setFiltroConfirmacao('')} style={{padding:'4px 10px',borderRadius:14,border:'none',cursor:'pointer',fontSize:10,fontWeight:600,background:filtroConfirmacao===''?'#1e40af':'#e2e8f0',color:filtroConfirmacao===''?'#fff':'#475569'}}>Todos</button><button onClick={()=>setFiltroConfirmacao('pendente')} style={{padding:'4px 10px',borderRadius:14,border:'none',cursor:'pointer',fontSize:10,fontWeight:600,background:filtroConfirmacao==='pendente'?'#dc2626':'#fee2e2',color:filtroConfirmacao==='pendente'?'#fff':'#dc2626'}}>⏳ Pendente</button><button onClick={()=>setFiltroConfirmacao('social')} style={{padding:'4px 10px',borderRadius:14,border:'none',cursor:'pointer',fontSize:10,fontWeight:600,background:filtroConfirmacao==='social'?'#16a34a':'#dcfce7',color:filtroConfirmacao==='social'?'#fff':'#16a34a'}}>✅ Social</button><button onClick={()=>setFiltroConfirmacao('promorar')} style={{padding:'4px 10px',borderRadius:14,border:'none',cursor:'pointer',fontSize:10,fontWeight:600,background:filtroConfirmacao==='promorar'?'#7c3aed':'#ede9fe',color:filtroConfirmacao==='promorar'?'#fff':'#7c3aed'}}>✅ Promorar</button><button onClick={()=>setFiltroConfirmacao('admin')} style={{padding:'4px 10px',borderRadius:14,border:'none',cursor:'pointer',fontSize:10,fontWeight:600,background:filtroConfirmacao==='admin'?'#0369a1':'#e0f2fe',color:filtroConfirmacao==='admin'?'#fff':'#0369a1'}}>✅ Admin</button></div><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="🔍 Buscar nome, selo ou comunidade..."
