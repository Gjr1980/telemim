import { useState, useEffect } from "react";
    const{data:cp}=await supabase.from('contas_pagar').select('*').eq('status','pendente').order('criado_em',{ascending:false});
    if(cp)setContasPagar(cp);
    const{data:ch2}=await supabase.from('contas_pagar').select('*').eq('status','pago').order('pago_em',{ascending:false}).limit(20);
    if(ch2)setContasHist(ch2);
...
export default function App(){
  ...
  async function pagarConta(cid){
    ...
    const agora=new Date().toISOString();
    const{error:ep}=await supabase.from('contas_pagar').update({status:'pago',pago_em:agora}).eq('id',cid);
    if(ep){alert('Erro:'+ep.message);return;}
    const paga=contasPagar.find(x=>x.id===cid);
    setContasPagar(prev=>prev.filter(x=>x.id!==cid));
    if(paga)setContasHist(prev=>[{...paga,status:'pago',pago_em:agora},...prev.slice(0,19)]);
    setFlash('✅ Conta paga!');
  }
  async function criarConta(evt){
    ...
    const{data:nd,error:ec}=await supabase.from('contas_pagar').insert([rowData]).select();
    if(ec){alert('Erro:'+ec.message);return;}
    if(nd)setContasPagar(prev=>[nd[0],...prev]);
    ...
    <div style={{minHeight:"100vh",background:COLORS.bg,fontFamily:"'Segoe UI',system-ui,sans-serif",color:COLORS.text,paddingBottom:50}}>
      ...
      <div style={{maxWidth:640,margin:"0 auto",padding:"0 12px"}}>
        ...
        <div style={{paddingBottom:80}}>
          ...
          {(function(){var hj=new Date();var anoMes=hj.toISOString().slice(0,7);var diasMes=(mudancas||[]).filter(function(m){return m.data&&m.data.slice(0,7)===anoMes;});var diasU=[...new Set(diasMes.map(function(m){return m.data;}))];var totM3=diasMes.reduce(function(s,m){return s+(parseFloat(m.medicao)||0);},0);var brMes=diasU.length*RULES.van1a+totM3*RULES.medicaoPorM3;var impMes=brMes*RULES.imposto;var lqMes=brMes-impMes;var cstMes=custosDiarios.filter(function(cd){return cd.data&&cd.data.slice(0,7)===anoMes;});var cvMes=diasU.length*RULES.vanCusto;var cajMes=cstMes.reduce(function(s,cd){var n=diasMes.filter(function(m){return m.data===cd.data;}).length;return s+(cd.ajudantes>0?(RULES.aj1a+(n>0?n-1:0)*RULES.ajAdd)*(cd.ajudantes):0);},0);var calmMes=cstMes.reduce(function(s,cd){return s+(parseFloat(cd.custo_almoco)||0);},0);var ctMes=cvMes+cajMes+calmMes;var luMes=lqMes-ctMes;var mg=brMes>0?((luMes/brMes)*100).toFixed(1):'0.0';var fR=function(v){return 'R$ '+v.toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2});};var fN=function(v){return 'R$'+Math.round(v).toLocaleString('pt-BR');};return(<><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,padding:'20px 12px 10px',background:'#f8fafc'}}>{[{l:'Fat. Bruto',v:fR(brMes),ic:'💰',cor:'#1e40af',bg:'#eff6ff'},{l:'Rec. Líquida',v:fR(lqMes),ic:'✅',cor:'#047857',bg:'#f0fdf4'},{l:'Custo Total',v:fR(ctMes),ic:'💸',cor:'#b45309',bg:'#fffbeb'},{l:'Lucro',v:fR(luMes),ic:'🚀',cor:luMes>=0?'#047857':'#dc2626',bg:luMes>=0?'#f0fdf4':'#fef2f2'}].map(function(k,i){return(<div key={i} style={{background:k.bg,borderRadius:14,padding:'14px 12px',border:'1.5px solid '+k.cor+'33'}}><div style={{fontSize:20,marginBottom:4}}>{k.ic}</div><div style={{fontSize:11,color:'#64748b',marginBottom:2}}>{k.l}</div><div style={{fontSize:15,fontWeight:800,color:k.cor}}>{k.v}</div></div>);})}</div><div style={{display:'flex',gap:8,padding:'0 12px 12px',background:'#f8fafc'}}>{[{l:'Dias',v:diasU.length,ic:'📅'},{l:'Mudanças',v:diasMes.length,ic:'🚛'},{l:'m³',v:totM3.toFixed(0),ic:'📏'},{l:'Margem',v:mg+'%',ic:'📊'}].map(function(k,i){return(<div key={i} style={{flex:1,background:'#fff',borderRadius:10,padding:'10px 4px',textAlign:'center',boxShadow:'0 1px 4px rgba(0,0,0,0.06)'}}><div style={{fontSize:14}}>{k.ic}</div><div style={{fontSize:9,color:'#94a3b8',margin:'2px 0'}}>{k.l}</div><div style={{fontSize:12,fontWeight:700,color:'#1e293b'}}>{k.v}</div></div>);})}</div><div style={{padding:'0 12px 10px',background:'#f8fafc'}}><div style={{background:'#fff',borderRadius:14,padding:16,boxShadow:'0 2px 8px rgba(0,0,0,0.06)'}}><div style={{background:'#fff',borderRadius:14,padding:16,boxShadow:'0 2px 8px rgba(0,0,0,0.06)',marginBottom:12}}><div style={{fontWeight:800,fontSize:15,color:'#1e293b',marginBottom:12}}>📊 Despesa Semanal</div>{(function(){var hj3=new Date();var dow3=hj3.getDay();var sg3=new Date(hj3);sg3.setDate(hj3.getDate()-dow3+(dow3===0?-6:1));var dias3=Array.from({length:7},function(_,ii){var dd3=new Date(sg3);dd3.setDate(sg3.getDate()+ii);return dd3.toISOString().slice(0,10);});var tCam=0,tVan=0,tAj=0,tAlm=0;var rows3=dias3.map(function(dia3){var cdx3=custosDiarios.find(function(x){return x.data===dia3;});var nMud3=mudancas.filter(function(m){return m.data===dia3;}).length;if(nMud3===0)return null;var vCam3=cdx3&&cdx3.caminhao?parseFloat(cdx3.caminhao)||0:0;var vVan3=RULES.vanCusto||0;var vAj3=cdx3&&cdx3.ajudantes>0?(RULES.aj1a+(nMud3>1?nMud3-1:0)*RULES.ajAdd)*cdx3.ajudantes:0;var vAlm3=cdx3&&cdx3.custo_almoco?parseFloat(cdx3.custo_almoco)||0:0;tCam+=vCam3;tVan+=vVan3;tAj+=vAj3;tAlm+=vAlm3;var nm3=['Ｄom','Ｓeg','Ｔer','Ｑua','Ｑui','Ｓex','Ｓáb'][new Date(dia3+'T12:00:00').getDay()];var df3=dia3.slice(8)+'/'+dia3.slice(5,7);var isHj3=dia3===hj3.toISOString().slice(0,10);return(<div key={dia3} style={{display:'flex',alignItems:'center',padding:'8px 0',borderBottom:'1px solid #f1f5f9'}}><div style={{width:44,flexShrink:0}}><div style={{fontSize:10,color:isHj3?'#3b82f6':'#94a3b8',fontWeight:700}}>{nm3}</div><div style={{fontSize:13,fontWeight:800,color:isHj3?'#1e40af':'#475569'}}>{df3}</div></div><div style={{flex:1,display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr',gap:2,textAlign:'center'}}><div><div style={{fontSize:9,color:'#7c3aed'}}>🚛</div><div style={{fontSize:11,fontWeight:700,color:'#7c3aed'}}>R${Math.round(vCam3)}</div></div><div><div style={{fontSize:9,color:'#1e40af'}}>🚐</div><div style={{fontSize:11,fontWeight:700,color:'#1e40af'}}>R${Math.round(vVan3)}</div></div><div><div style={{fontSize:9,color:'#047857'}}>👷</div><div style={{fontSize:11,fontWeight:700,color:'#047857'}}>R${Math.round(vAj3)}</div></div><div><div style={{fontSize:9,color:'#b45309'}}>🍽</div><div style={{fontSize:11,fontWeight:700,color:'#b45309'}}>R${Math.round(vAlm3)}</div></div></div></div>);});var totSem3=tCam+tVan+tAj+tAlm;return(<><div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr',gap:6,marginBottom:10,paddingBottom:10,borderBottom:'2px solid #e2e8f0'}}><div style={{textAlign:'center',background:'#f5f3ff',borderRadius:8,padding:'8px 4px'}}><div style={{fontSize:16}}>🚛</div><div style={{fontSize:9,color:'#94a3b8'}}>Caminhão</div><div style={{fontSize:13,fontWeight:800,color:'#7c3aed'}}>R${Math.round(tCam)}</div></div><div style={{textAlign:'center',background:'#eff6ff',borderRadius:8,padding:'8px 4px'}}><div style={{fontSize:16}}>🚐</div><div style={{fontSize:9,color:'#94a3b8'}}>Van</div><div style={{fontSize:13,fontWeight:800,color:'#1e40af'}}>R${Math.round(tVan)}</div></div><div style={{textAlign:'center',background:'#f0fdf4',borderRadius:8,padding:'8px 4px'}}><div style={{fontSize:16}}>👷</div><div style={{fontSize:9,color:'#94a3b8'}}>Ajudante</div><div style={{fontSize:13,fontWeight:800,color:'#047857'}}>R${Math.round(tAj)}</div></div><div style={{textAlign:'center',background:'#fffbeb',borderRadius:8,padding:'8px 4px'}}><div style={{fontSize:16}}>🍽</div><div style={{fontSize:9,color:'#94a3b8'}}>Almoço</div><div style={{fontSize:13,fontWeight:800,color:'#b45309'}}>R${Math.round(tAlm)}</div></div></div><div style={{display:'flex',justifyContent:'space-between',marginBottom:8,fontSize:11,color:'#475569'}}><span>Dias com mudanças: {rows3.filter(Boolean).length}</span><strong>Total: R$ {totSem3.toLocaleString('pt-BR',{minimumFractionDigits:2})}</strong></div>{rows3.filter(Boolean)}</> );})()}</div>
          <div class="header-meta"><div>CONTRATO: PROMORAR</div><div>Gerado: ${new Date().toLocaleDateString("pt-BR")} ${new Date().toLocaleTimeString("pt-BR")}</div></div>
        </div>
        <div style="margin-top:10px;font-size:13px;color:#e67e22;font-weight:700">📅 Período: ${periodo}</div>
      </div>
