...
export default function App(){
  ...
  const [filtroAprov,setFiltroAprov]=useState("todos");
  ...
  const filtered=[...mudancas].filter(function(mx){if(mx.deleted_at)return false;if(filtroAprov==="social"&&!mx.approved_by_social)return false;if(filtroAprov==="promorar"&&!mx.approved_by_promorar)return false;if(filtroAprov==="admin"&&!mx.approved_by_admin)return false;if(filtroAprov==="pendente"&&(mx.approved_by_social||mx.approved_by_promorar||mx.approved_by_admin))return false;
      ...
      {tab==="lista"&&(
        ...
        <div style={{paddingBottom:80}}>
          <div style={{display:'flex',gap:6,padding:'12px 12px 0',background:'#f8fafc'}}>{[{v:'semana',l:'Semana'},{v:'mes_atual',l:'Mês Atual'},{v:'mes_ant',l:'Mês Anterior'}].map(function(p){return(<button key={p.v} onClick={()=>setPeriodoFin(p.v)} style={{flex:1,padding:'8px 2px',borderRadius:10,border:'none',background:periodoFin===p.v?'#1e40af':'#e2e8f0',color:periodoFin===p.v?'#fff':'#475569',fontSize:11,fontWeight:periodoFin===p.v?700:500,cursor:'pointer'}}>{p.l}</button>);})}</div><div style={{background:'linear-gradient(135deg,#1e293b,#1e40af)',padding:'20px 16px 24px',marginBottom:-12}}><div style={{fontSize:12,color:'rgba(255,255,255,0.65)',marginBottom:2}}>Painel Financeiro</div><div style={{fontSize:21,fontWeight:800,color:'#fff'}}>{(function(){if(periodoFin==='semana'){var d=new Date();var ds=d.getDay();var s0=new Date(d);s0.setDate(d.getDate()-ds+(ds===0?-6:1));var s1=new Date(s0);s1.setDate(s0.getDate()+6);var fmt=function(dt){return dt.getDate()+'/'+(dt.getMonth()+1);};return 'Semana: '+fmt(s0)+' a '+fmt(s1)+'/'+s1.getFullYear();}if(periodoFin==='mes_ant'){var dm=new Date();dm.setDate(1);dm.setMonth(dm.getMonth()-1);return dm.toLocaleDateString('pt-BR',{month:'long',year:'numeric'}).replace(/^\w/,function(s){return s.toUpperCase();});}return new Date().toLocaleDateString('pt-BR',{month:'long',year:'numeric'}).replace(/^\w/,function(s){return s.toUpperCase();});})()}</div></div>
           ...
           {tab==="financeiro"&&isAdmin&&(()=>{var _hjM=new Date();var _anoMesM=_hjM.getFullYear()+"-"+String(_hjM.getMonth()+1).padStart(2,"0");var _nomeMesM=_hjM.toLocaleDateString("pt-BR",{month:"long",year:"numeric"}).replace(/^\w/,s=>s.toUpperCase());var _semsM=[];var _diaMesM=new Date(_hjM.getFullYear(),_hjM.getMonth(),1);while(_diaMesM.getMonth()===_hjM.getMonth()){  var _dwM=_diaMesM.getDay();var _diffM=_dwM===0?6:_dwM-1;  var _s0M=new Date(_diaMesM);_s0M.setDate(_diaMesM.getDate()-_diffM);  var _s1M=new Date(_s0M);_s1M.setDate(_s0M.getDate()+6);  var _siM=_fmtDate(_s0M);var _sfM=_fmtDate(_s1M);  if(!_semsM.find(function(x){return x.si===_siM;}))_semsM.push({si:_siM,sf:_sfM});  _diaMesM.setDate(_diaMesM.getDate()+7);}var _fD4=function(d){if(!d)return "";var p=d.split("-");return p[2]+"/"+p[1];};var _fV4=function(v){return "R$ "+parseFloat(v||0).toLocaleString("pt-BR",{minimumFractionDigits:2});};var _tMes=["caminhao","van","ajudante","almoco"];var _lbl4={"caminhao":"Caminhão","van":"Van","ajudante":"Ajudante","almoco":"Almoço"};var _ico4={"caminhao":"🚚","van":"🚐","ajudante":"👷","almoco":"🍽️"};var _cor4={"caminhao":"#92400e","van":"#1e40af","ajudante":"#065f46","almoco":"#7c3aed"};return(<div style={{padding:"12px 12px 4px"}}><div style={{fontWeight:800,fontSize:14,color:"#1e293b",marginBottom:10,display:"flex",alignItems:"center",gap:6}}><span>📊</span> Custos por Semana — {_nomeMesM}</div>{_semsM.map(function(_sM){  var _itsM=contasSemana.filter(function(x){return x.semana_inicio===_sM.si;});  var _totM=_itsM.reduce(function(s,x){return s+(parseFloat(x.valor_editado||x.valor_calculado)||0);},0);  var _isAtual=_sM.si<=_fmtDate(_hjM)&&_sM.sf>=_fmtDate(_hjM);  return(  <div key={_sM.si} style={{background:_isAtual?"#eff6ff":"#fff",border:_isAtual?"1.5px solid #3b82f6":"1px solid #e2e8f0",borderRadius:12,padding:"10px 14px",marginBottom:8,boxShadow:"0 1px 4px rgba(0,0,0,0.05)"}}>    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>      <span style={{fontWeight:700,fontSize:12,color:_isAtual?"#1e40af":"#64748b"}}>        {_isAtual&&<span style={{background:"#dbeafe",color:"#1e40af",borderRadius:4,padding:"1px 5px",fontSize:10,marginRight:4}}>ATUAL</span>}        📆 {_fD4(_sM.si)} a {_fD4(_sM.sf)}      </span>      <span style={{fontWeight:800,fontSize:13,color:_totM>0?"#dc2626":"#94a3b8"}}>{_fV4(_totM)}</span>    </div>    {_tMes.map(function(_tp){      var _it=_itsM.find(function(x){return x.tipo===_tp;});      var _vl=parseFloat((_it&&(_it.valor_editado||_it.valor_calculado))||0);      if(_vl===0)return null;      return(<div key={_tp} style={{display:"flex",alignItems:"center",gap:6,padding:"2px 0"}}>        <span style={{fontSize:12}}>{_ico4[_tp]}</span>        <span style={{fontSize:11,color:"#64748b",flex:1}}>{_lbl4[_tp]}</span>        <span style={{fontSize:12,fontWeight:600,color:_cor4[_tp]}}>{_fV4(_vl)}</span>      </div>);    })}  </div>);})}</div>);})()}
{tab==="financeiro"&&isAdmin&&(()=>{
var _tipos2=[{tp:"caminhao",ico:"🚚",lbl:"Caminhão"},{tp:"van",ico:"🚐",lbl:"Van"},{tp:"ajudante",ico:"👷",lbl:"Ajudante"},{tp:"almoco",ico:"🍽️",lbl:"Almoço"}];
var _semFin=[];
var _hjFin=new Date();
for(var _wi=1;_wi<=8;_wi++){var _bFin=new Date(_hjFin);_bFin.setDate(_hjFin.getDate()-(_wi*7));var _dwFin=_bFin.getDay();var _diffFin=_dwFin===0?6:_dwFin-1;var _s0Fin=new Date(_bFin);_s0Fin.setDate(_bFin.getDate()-_diffFin);var _s1Fin=new Date(_s0Fin);_s1Fin.setDate(_s0Fin.getDate()+6);_semFin.push({si:_fmtDate(_s0Fin),sf:_fmtDate(_s1Fin)});}
...
<div style={{fontWeight:800,fontSize:14,color:"#1e293b",marginBottom:10,display:"flex",alignItems:"center",gap:6}}><span>📅</span> Editar Custos — Semanas Anteriores</div>
...
<div style={{fontWeight:700,fontSize:12,color:"#64748b",marginBottom:8}}>📆 {_fD3(_sem2.si)} a {_fD3(_sem2.sf)}</div>
